        ...
        <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-16">
          ...
          <Reveal>
            <div className="relative mx-auto w-full max-w-[280px]">
              <span aria-hidden="true" className="pointer-events-none absolute -inset-6 rounded-[3rem] bg-gold/10 blur-3xl" />
              <div className="relative rounded-[2.2rem] border-[6px] border-[#23262e] bg-onyx-1 p-1 shadow-[0_40px_90px_-30px_rgba(0,0,0,0.9)]">
          ...
          <div>
            <Reveal>
              ...
              <h2 className="mt-4 font-display text-[clamp(1.7rem,4.5vw,2.4rem)] font-medium leading-tight text-paper">
                Know exactly how your child is doing.
              </h2>
              <p className="mt-3 max-w-md text-[0.95rem] leading-relaxed text-paper/70 sm:text-base">
              ...
              <ul className="mt-6 space-y-3.5">
                {INSIDE.map((x) => (
                  <li key={x.t} className="flex items-start gap-3">
                    ...
                    <span>
                      <span className="block text-[0.95rem] font-semibold text-paper">{x.t}</span>
                      <span className="block text-sm text-paper/60">{x.d}</span>
                    </span>
                  </li>
                ))}
              </ul>
              <Link href="/parent/login"
                className="mt-7 inline-flex items-center justify-center gap-2 rounded-full border border-emerald-400/40 bg-emerald-400/[0.06] px-6 py-3.5 text-sm font-semibold text-emerald-200 transition-colors hover:border-emerald-400/70">
