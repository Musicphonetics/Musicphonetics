...
export function LoginChooser({ align = "center" }: { align?: "center" | "start" }) {
  ...
  return (
    <div ref={ref} className="relative inline-block">
      <button
        ...
        aria-haspopup="true"
        aria-expanded={open}
        className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/[0.04] px-6 py-3 text-sm font-semibold text-paper transition-colors hover:border-white/40"
      >
      ...
      {open && (
        <div className={`absolute z-40 mt-2 w-64 overflow-hidden rounded-2xl border border-white/12 bg-onyx-1 p-1.5 shadow-2xl ${align === "center" ? "left-1/2 -translate-x-1/2" : "left-0"}`}>
          <p className="px-3 pb-1.5 pt-2 text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-paper/45">Who&apos;s logging in?</p>
          <Link href="/parent/login" onClick={() => setOpen(false)}
            ...
            <span>
              <span className="block text-sm font-semibold text-paper">Parent / Student</span>
              <span className="block text-xs text-paper/55">Classes, progress &amp; fees</span>
            </span>
          </Link>
          <Link href="/teacher/login" onClick={() => setOpen(false)}
            className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-left hover:bg-white/5">
            <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full border border-gold/30 bg-gold/[0.1] text-gold">
            ...
            <span>
              <span className="block text-sm font-semibold text-paper">Teacher</span>
              <span className="block text-xs text-paper/55">Your students &amp; class updates</span>
            </span>
          </Link>
        </div>
      )}
