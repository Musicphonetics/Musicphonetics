...
export default function SalesLogin() {
  ...
  useEffect(() => {
    ...
    if (!client) return;
    client.auth.getSession().then(({ data }) => { if (data.session) router.replace("/sales/leads"); });
  }, [router]);

  async function login() {
    setMsg(""); setBusy(true);
    const { client, error } = getSupabaseSafe();
    if (!client) { setBusy(false); setMsg(error || "Not configured"); return; }
    try {
      const { error: authErr } = await client.auth.signInWithPassword({ email: email.trim(), password });
      setBusy(false);
      if (authErr) setMsg(authErr.message);
      else router.replace("/sales/leads");
  ...
  return (
    <div className="grid min-h-screen place-items-center bg-ink px-5 py-10 text-paper">
      <div className="w-full max-w-sm">
        ...
        <h1 className="mt-6 text-center font-display text-2xl font-semibold">Sales Workspace</h1>
        <p className="mt-1.5 text-center text-sm text-paper/60">Musicphonetics lead department.</p>
        {!isSupabaseConfigured() && (
          <p className="mt-6 rounded-xl border border-red-400/40 bg-red-500/10 p-3 text-center text-sm text-red-200">Not configured yet.</p>
        )}
        <form className="mt-7 space-y-3 rounded-2xl border border-white/12 bg-white/[0.04] p-5"
          onSubmit={(e) => { e.preventDefault(); login(); }}>
          <input className={input} type="email" inputMode="email" autoComplete="email" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} />
          ...
          <button type="submit" disabled={busy} className="w-full rounded-full bg-gold py-3.5 text-base font-semibold text-ink disabled:opacity-60">
            {busy ? "Signing in…" : "Sign in"}
          </button>
          {msg && <p className="text-center text-sm text-paper/75">{msg}</p>}
          <button type="button" onClick={forgot} className="w-full text-center text-xs font-medium text-gold/90 hover:text-gold">Forgot password?</button>
        </form>
      </div>
    </div>
  );
