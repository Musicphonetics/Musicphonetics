...
import {
  loadLeads, loadLeadStats, LEAD_STATUSES, LEAD_STATUS_LABEL, LEAD_STATUS_TONE,
  type LeadListRow,
...
export default function OwnerLeads() {
  ...
  const refresh = useCallback(async () => {
    const [{ rows, total, error }, s] = await Promise.all([
      loadLeads({ page, pageSize: PAGE_SIZE, status, assigned, source, followUpDue, search }),
      loadLeadStats().catch(() => ({})),
    ]);
    setErr(error); setRows(rows); setTotal(total); setStats(s); setSel(new Set());
  ...
  return (
    <PortalShell role="owner" tabs={OWNER_TABS} variant="wide" title="Leads">
      ...
      {openId && (
        <LeadDetailPanel leadId={openId} teachers={teachers} onClose={() => setOpenId(null)} onChanged={refresh} />
      )}
    </PortalShell>
  );
}
