...
export default function OwnerLogin() {
  ...
  useEffect(() => {
    ...
    if (!client) return;
    client.auth.getSession().then(({ data }) => { if (data.session) router.replace("/owner/dashboard"); });
  }, [router]);
  ...
  return (
    <div className="grid min-h-screen place-items-center bg-ink px-5 py-10 text-paper">
      <div className="w-full max-w-sm">
        ...
        <h1 className="mt-6 text-center font-display text-2xl font-semibold">Owner Dashboard</h1>
        <p className="mt-1.5 text-center text-sm text-paper/60">Musicphonetics command centre.</p>
