  ...
  const childMenu = student && data ? (
    <div className="relative">
      <button type="button" onClick={() => data.students.length > 1 && setMenu((v) => !v)}
        className="flex items-center gap-2 rounded-full border border-hairline bg-white py-1 pl-1 pr-2.5"
        aria-haspopup={data.students.length > 1} aria-expanded={menu}>
        <span className="grid h-8 w-8 place-items-center rounded-full border border-gold/40 bg-gold/15 font-display text-sm font-bold text-[#7A5E0F]">
          {firstName(student.name).charAt(0)}
        </span>
        <span className="max-w-[6rem] truncate text-sm font-semibold text-ink">{firstName(student.name)}</span>
      ...
      {menu && data.students.length > 1 && (
        <div className="absolute right-0 top-11 z-40 w-44 overflow-hidden rounded-2xl border border-hairline bg-white shadow-xl">
          {data.students.map((s, i) => (
            <button key={s.id} onClick={() => { setIdx(i); setMenu(false); }}
              className={cn("flex w-full items-center gap-2 px-3 py-2.5 text-left text-sm", i === idx ? "bg-gold/10 text-[#7A5E0F]" : "text-ink/80 hover:bg-paper")}>
              <span className="grid h-6 w-6 place-items-center rounded-full bg-gold/15 text-xs font-bold text-[#7A5E0F]">{firstName(s.name).charAt(0)}</span>
              {s.name}
            </button>
          ))}
        </div>
